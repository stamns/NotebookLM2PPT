...
from src.pdf2png import pdf_to_png
from src.utils.image_viewer import show_image_fullscreen
from src.utils.screenshot_automation import take_fullscreen_snip, mouse, screen_height, screen_width
from src.ppt_combiner import combine_ppt
    ...
    for idx, png_file in enumerate(png_files, 1):
        ...
        try:
            ...
            if success and ppt_filename:
                ...
                # 从下载文件夹查找并复制PPT文件
                # PPT窗口标题格式通常为: "文件名 - PowerPoint" 或 "文件名.pptx - PowerPoint"
                # 提取文件名（移除 " - PowerPoint" 后缀）
                ...
                if ppt_source_path.exists():
                    # 目标文件名：与png相同的名字，但扩展名为.pptx
                    target_filename = png_file.stem + ".pptx"  # page_0001.pptx
                ...
                mouse.click(button='left', coords=close_button) # 点击取消按钮关闭截图工具
        except Exception as e:
...
使用示例:
  ...
  python main.py examples/demo.pdf                  # 转换指定PDF
  python main.py --no-inpaint                       # 禁用图像修复（去水印）
  python main.py -d 3 -t 60                         # 设置延迟和超时
  python main.py -s 0.9 examples/demo.pdf           # 设置显示尺寸比例
        """
    )
    ...
    parser.add_argument(
        'pdf_file',
        nargs='?',
        default='examples/floyd.pdf',
        help='PDF 文件路径（默认: examples/floyd.pdf）'
    )
    ...
    # 因为pdf一定是16：9，所以这里直接用16：9的比例来计算，屏幕不一定是16：9
    # 尽量填满屏幕
    ...
    # if args.inpaint:
    #     assert args.dpi == 150, "启用图像修复时，DPI 必须为 150"
